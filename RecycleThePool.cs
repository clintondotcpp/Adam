...
public class Bullet : MonoBehaviour
{
    ...
    //We need to know when a gameobject is turned or off. For on we use OnEnable which is automatically called by unity, anytime the gameobject becomes enabled
    //When that happens we need to invoke, we use a method called invoke
    void OnEnable()
    {
        Invoke("Hide", 1); //this invokes the method Hide in 0 seconds, which is immediately
    }

    void Hide()
    {
        //What happens when we hide a game object, we recycle the gameObject
        //Re-cycle the gameobject
        //To re-cycle or request, all we have to do is turn it off, because in the method RequestBullet in the PoolManager script, we use a gameobject by setting to setactive visible
        this.gameObject.SetActive(false); //by doing this, it is automatically going to be re-cycled
        Debug.Log("Hi");
    }
